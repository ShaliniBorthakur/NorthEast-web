...
def checkout(request):
    if request.method=="POST":
        ...
        for i in list(loads.values()):
            ...
            prod.quantity_ever_sold += int(i[0])
            prod.save() 
    ...
    def post(self, request):
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                post.quantity_ever_stocked = form.data['quantity']
...
class EditItemView(UserPassesTestMixin, LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        qty = item.get_quantity_available() 
        ...
        if request.method == 'POST':            
            ...
            if form.is_valid():
                ...
                if qty is None:
                    ...
                    post.quantity_ever_stocked += int(qty)
                    post.quantity = int(qty)
                    # post.quantity_ever_sold = post.quantity_sold
